...
class Node():
    ...
    def __init__(self, name=None, mRNA=None, gRNA=None, gRNA_position=None, edited_until=None, latest_start=None,
                 orange=False, green=False, depth=None):
        ...
        # Track where this round of editing started to keep 3'-5'
        # Was originally using min(gRNA positions) but this doesn't keep it using the latest block's position
        # E.g. if gRNAs have bound at 123, 124, 125 - it would use 123 even if the latest one bound at 125
    ...
    def add_edge(self, target_node, new_gRNA_name, anchorsize=None, thermo_probability=None):
        ...
        new_edges_name = self.name + "^to^" + target_node.name + "^with^" + new_gRNA_name + "^thermo_probability_" + \
                         str(thermo_probability) + "^anchor_" + str(anchorsize)
        ...
        if new_edges_name in list(self.outgoing_edges.keys()):
            ...
            self.outgoing_edges[new_edges_name] = Edge(start=self, target=target_node, gRNA=new_gRNA_name,
                                                       anchorsize=anchorsize, thermo_probability=thermo_probability)

    # Needed for statistics module, not used in simulations.
    def add_incoming_edge(self, source_node, new_gRNA_name, thermo_probability=None):
        assert isinstance(source_node, Node), \
            "Please provide a valid Node object as target for the new edge. Currently: " + str(type(source_node))
        assert isinstance(new_gRNA_name, str), \
            "Please provide a valid gRNA name String for the new edge. Currently: " + str(type(new_gRNA_name))

        new_edges_name = source_node.name + "^to^" + self.name + "^with^" + new_gRNA_name + "^thermo_probability" + \
                         "_" + str(thermo_probability)

        # If edge already exists increase number of occurrences. If not, add edge.
        # TODO: update weight of edge?
        if new_edges_name in list(self.incoming_edges.keys()):
            new_edge = self.incoming_edges[new_edges_name]
            new_edge.increment_occurrence()
        else:
            self.incoming_edges[new_edges_name] = Edge(start=source_node, target=self, gRNA=new_gRNA_name,
                                                       thermo_probability=thermo_probability)
    ...
    #
    def update_edited_position(self, new_start_position, new_edited_until):
        self.latest_start = new_start_position
    ...
    def normalise_weights(self):
       # thermo_total = 0
        ...
        for edge in self.outgoing_edges.values():
            ...
            anchor_total += edge.anchor_size
        for edge in self.outgoing_edges.values():
            edge.anchor_weight = edge.anchor_size / anchor_total
            edge.probability = edge.anchor_weight * edge.thermo_probability
            if edge.probability > 1:
                edge.probability = 1
            probability_total += edge.probability
        for edge in self.outgoing_edges.values():
            edge.normalised_probability = edge.probability / probability_total
            edge.name = edge.name + "^normalised_p" + str(edge.normalised_probability)
            ...
            if edge.target.probability is None:
                edge.target.probability = (edge.normalised_probability * self.probability)
            else:
                edge.target.probability += (edge.normalised_probability * self.probability)
                # P shouldn't exceed 1, but in some cases with multiple incoming nodes it can. Prevent this.
            if edge.target.probability > 1:
                edge.target.probability = 1
...
class Edge():
    def __init__(self, start=None, target=None, gRNA=None, anchorsize=None, thermo_probability=None):
        ...
        self.occurrences = 1
        # Name of edge
        self.name = self.start.name + "^to^" + self.target.name + "^with^" + self.gRNA + "^thermo_probability_" + str(thermo_probability) + "^anchor_" + str(anchorsize)
        # Weights and probability
        self.thermo_probability = thermo_probability
        self.anchor_size = anchorsize
        self.anchor_weight = None
        self.probability = None
        self.normalised_probability = None
...
class Network():
    ...
    # Remove a node (specifically for white node investigations
    ...
    # return list of all white nodes, excluding root
    def get_white_nodes(self):
        white_node_list = []
        total_node_count = 0
        for node in self.get_nodes():
            total_node_count += 1
            if not node.green and not node.orange and node.name != '"unedited mRNA"':
                white_node_list.append(node)

        return white_node_list
        ...
        for node in self.get_nodes():
            ...
            for edge in list(node.outgoing_edges.values()):
                if edge.occurrences > 1:
                    ...
                    #TODO: for now, removed P from graph so edges easier to generate.
