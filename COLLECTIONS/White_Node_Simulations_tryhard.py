from COLLECTIONS import AnchorAligners, Editors, BF_Network
import sys, time, os
...
class Simulation():
    ...
    def add_network(self, network):
        assert isinstance(network, BF_Network.Network), "Please provide a valid Network object. Current type is " + str(
            type(network)) + "."
    ...
    def add_root(self, root):
        assert isinstance(root, BF_Network.Node), "Only Node-objects can be root."  # maybe specific graphnode object?
        self.root = root
    ...
    def initialize(self, filepath_gene, filepath_control, filepath_RNAs, filepath_nodes, second_round=False):
        ...
        network = BF_Network.Network(name=self.name)
        ...
        root = BF_Network.Node(name='"unedited mRNA"', mRNA=self.mRNA_unedited, edited_until=len(self.mRNA_unedited))
                ...
                while current_position < len(current_node.mRNA) - anchor_min:
                    ...
                    if anchor_size:
                        ...
                        if editing_type == "Uinsert":
                            ...
                            new_mRNAs = Editors.middle_slidingthermo_editing(current_node.mRNA, gRNA[1], current_position,
    ...
    def simulation_editing_area_only(self, editing_type, graph_type, position, anchor_min, anchor_max, depth,
        ...
        while self.to_visit:
            ...
            for gRNA in self.gRNAs:
                ...
                while current_position < len(current_node.mRNA) - anchor_min and current_position < end_of_editingarea:
            ...
            current_node.normalise_weights()
            print("normalised, edge count:", len(current_node.outgoing_edges))
            # only add the highest probability white node
            max_P = 0
            best_white_node = None
            best_white_edge = None
            for edge in current_node.temp_edges.values():
                if not edge.target.green and not edge.target.orange:
                    if edge.target.probability > max_P:
                        ...
                        best_white_edge = edge

            if best_white_node:
                current_node.add_edge(best_white_node, best_white_edge.gRNA, best_white_edge.thermo_probability, best_white_edge.normalised_probability)
                #print("white", best_white_node)
                if best_white_node not in self.to_visit and best_white_node not in self.visited:
                    self.to_visit.append(best_white_node)
                    print("best white", best_white_node)

            print('after white', len(current_node.outgoing_edges))
            for name, edge in current_node.temp_edges.items():
                #print("before", edge.target.green, edge.target.orange)
                if edge.target.green or edge.target.orange:
                    current_node.add_edge(edge.target, edge.gRNA, edge.thermo_probability, edge.normalised_probability)
                    if edge.target.probability > threshold:
                        if edge.target not in self.to_visit and edge.target not in self.visited:
                    ...
                    if edge.target != best_white_node:
                        print(len(self.network.nodes))
                        self.network.remove_node(edge.target)
                        print("remove", len(self.network.nodes))
                        print("edges", len(current_node.outgoing_edges))

            count = 0
            for edge in current_node.outgoing_edges.values():
                if not edge.target.green and not edge.target.orange:
                    print("g", edge.target.green)
                    print("O", edge.target.orange)
                    count += 1
            assert count <= 1
            # Remove once done
            current_node.temp_edges = None
    ...
    def _add_child(self, new_mRNA, node, gRNA, thermo_score, current_position, position_after_editing, graph_type):
        ...
        if existing_node:
            ...
            existing_node.update_edited_position(position_after_editing)
        ...
        # If it does not exist yet, introduce a new node (depending on graph_type)
            ...
            if green:
                new_node = BF_Network.Node(name=new_node_name, mRNA=new_mRNA, gRNA=gRNA[0],
            ...
            # Colour nodes always allowed
            if green or orange:
                self.network.add_node(new_node)
                # Add edge from current node to this new node
                node.add_temp_edge(new_node, gRNA[0], thermo_score)
                #node.add_edge(new_node, gRNA[0], thermo_score
            # White nodes only allowed under certain conditions. First add to temp list to control how many there are. 
            elif graph_type != "ColourOnly":
                self.network.add_node(new_node)
                node.add_temp_edge(new_node, gRNA[0], thermo_score)
