from COLLECTIONS import BF_Simulations
import os
...
class Program():
    ...
    def set_filepaths(self, gene, control, gRNAs, nodes, output):
        assert isinstance(gene, str) and os.path.exists(gene), \
            "Please provide a valid path to the gene file. Wrong path: "+gene
        assert isinstance(control, str) and os.path.exists(control), \
            "Please provide a valid path to the control file. Wrong path: "+control
        assert isinstance(gRNAs, str) and os.path.exists(gRNAs), \
            "Please provide a valid path to the gRNAs file. Wrong path: "+gRNAs
        assert isinstance(nodes, str) and os.path.exists(nodes), \
            "Please provide a valid path to the nodes file. Wrong path: "+nodes
        assert isinstance(output, str) and os.path.exists(output), \
            "Please provide a valid path to the output directory. Wrong path: "+output
    ...
    def set_parameters(self, editing_type, simulation_type, graph_type, anchor_min, anchor_max, starting_position,
                       overlap=None, overhang=None, value_GC=None, value_AU=None, value_GU=None, value_mismatch=None,
                       windowsize=None, mismatches_allowed=None, threshold=None):
        ...
        assert (overlap is None and overhang is None) or overlap + overhang > 0, \
            "Please provide a valid window or no window."
    ...
    def run_simulation(self, used_gRNAs, used_gene, maximum_depth=None, second_round=False):
        ...
        # Create simulation
        self.simulation = BF_Simulations.Simulation(name=simulation_name)
        ...
        self.simulation.run(editing_type=self.editing_type, simulation_type=self.simulation_type,
                            ...
                            value_GC=self.value_GC, value_AU=self.value_AU, value_GU=self.value_GU,
                            value_mismatch=self.value_mismatch, end_of_editingarea=len(self.simulation.mRNA_unedited))

        # Save information that simulation was executed to allow printing and saving.
        self.simulation_finished = True

    def find_binding_sites(self, simulation_name):
        ...
        binding_sites = self.simulation.gRNAs_binding_unedited(anchor_min=self.anchor_min, anchor_max=self.anchor_max)
